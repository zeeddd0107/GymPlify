 ...
 * @param {string} mode - Mode (login, register, forgot-password, etc.)
 * @returns {Promise<object>} - Response with otpId and expiresAt
 */
export async function sendOTP(email, mode = 'login') {
 ...
 */
export async function verifyOTP(email, otp, otpId, mode = 'login') {
  try {
